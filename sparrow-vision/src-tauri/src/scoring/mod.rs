...
use crate::types::{
    AnalyzeHandRequest, ScoreBreakdown, ScoringResult, TsumoPoints, WindInput, YakuResult,
};
...
fn analyze_hand_inner(request: AnalyzeHandRequest) -> Result<ScoringResult, String> {
    ...
    let hand_tiles =
        request::normalize_closed_hand(&request.hand_tiles, &winning_tile_id, request.melds.len())?;
    let hand = hand_tiles
        .iter()
...
fn score_breakdown(
    points: &riichi_calc::calculator::result::Points,
    fu: u8,
    han: u8,
    is_yakuman: bool,
) -> ScoreBreakdown {
    ScoreBreakdown {
        ...
        tsumo_points: match points {
            riichi_calc::calculator::result::Points::ChildTumo(non_dealer, dealer) => {
                Some(TsumoPoints {
                    dealer: *dealer,
...
mod tests {
    use super::*;
    use crate::types::{
        MeldInput, MeldKind, RiichiInput, SpecialWinInput, WinMethodInput, WindInput,
    };
    ...
    fn scores_closed_tsumo_hand() {
        ...
        assert!(result
            .score_breakdown
            .and_then(|score| score.tsumo_points)
            .is_some());
    }
